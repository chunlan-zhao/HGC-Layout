...
*Contrastive graph priors and adaptive force-directed optimization for robust urban scene layouts from text and georeferenced context*
...
The stage structure builds on HOG-Layout (Jiang et al., 2026), replacing its indoor
coordinate predictor, fixed optimization weights and asset hierarchy with
geospatial graph-conditioned components.
...
Also included: four compared methods, all our own implementations rather than code
obtained from the original authors (`hgc_layout/baselines/`), the eleven metrics and
...
MIT. See `LICENSE`. Citation metadata is in `CITATION.cff`.# HGC-Layout
